...
export function statistics(games){
 ...
 for(const g of complete){
  ...
  for(const p of g.players){
   ...
   row.games++;row.wins+=Number(winnerIds.has(p.id));row.total+=v;row.best=Math.max(row.best,v);players.set(key,row);
  }
 }
