...
const KEY='catan-runde-prototype-v1';
...
function render(){
  const g=game(),p=actor(),done=submitted(g),host=state.actorId===g.hostId;
  ...
  root.innerHTML=`<header class="topbar"><div class="top-inner"><a class="brand" href="#overview" aria-label="Catan Runde – Spielübersicht"><span class="brand-mark" aria-hidden="true">C</span><span class="brand-text">Catan Runde<span class="brand-sub">Classic · Am Spieltisch</span></span></a><div class="top-label"><span>◇</span><span>Eure Runde. Eure Ergebnisse.</span></div></div></header>
    <div class="demo-bar"><div class="demo-inner"><strong>PROBELAUF</strong><span>Beispieldaten auf diesem Gerät. Spielerwechsel simuliert die Handys; noch keine Anmeldung oder Synchronisierung.</span></div></div>
    <div class="shell"><div class="page-heading"><div><p class="eyebrow">Gemeinsam am Spieltisch</p><h1>Unsere Partie</h1><p class="subtle">Jeder trägt ein. Die Spielleitung bestätigt.</p></div><button class="button" data-action="new"><span aria-hidden="true">＋</span> Neue Partie</button></div>
    <div class="workspace"><aside class="sidebar"><div class="identity"><label for="actor">Ansicht testen als</label><select id="actor">${g.players.map(x=>`<option value="${e(x.id)}" ${x.id===p.id?'selected':''}>${e(x.name)}${x.id===g.hostId?' · Spielleitung':''}</option>`).join('')}</select><p class="subtle">Demo-Wechsel · kein echter Login</p></div>
    <nav class="nav" aria-label="Partie"><button data-view="overview" class="${view==='overview'?'active':''}" ${view==='overview'?'aria-current="page"':''}><span class="nav-icon" aria-hidden="true">▦</span> Übersicht <span class="count">${done}/${g.players.length}</span></button><button data-view="entry" class="${view==='entry'?'active':''}" ${view==='entry'?'aria-current="page"':''}><span class="nav-icon" aria-hidden="true">✎</span> Meine Werte</button><button data-view="review" class="${view==='review'?'active':''}" ${view==='review'?'aria-current="page"':''}><span class="nav-icon" aria-hidden="true">✓</span> ${g.status==='confirmed'?'Ergebnis':'Prüfen'}</button></nav>
    ...
    <main id="main" tabindex="-1">${saveError?`<div class="error-list" role="alert">${e(saveError)}</div>`:''}${view==='overview'?overview(g,p):view==='entry'?entry(g,p):reviewView(g,p,host)}</main></div>
    <footer class="footer"><span>Lokaler Prototyp · Version 0.1</span><button class="muted-link" data-action="export">Daten exportieren ↓</button></footer></div>`;
}
function overview(g,p){
  const done=submitted(g),closed=g.status==='confirmed';
  return `${banner(g)}${closed?`<div class="notice success">Die Partie ist bestätigt. Alle Eingaben sind gesperrt.</div>`:''}<section class="panel" aria-labelledby="round-title"><div class="panel-header"><h2 id="round-title">Am Tisch</h2><span class="chip ${done===g.players.length?'green':'gold'}">${done} von ${g.players.length} fertig</span></div><p class="subtle">${closed?'Euer Ergebnis steht fest.':'Wer hat seinen Endstand schon eingetragen?'}</p><div class="progress-track" role="progressbar" aria-label="Abgeschlossene Eingaben" aria-valuenow="${done}" aria-valuemin="0" aria-valuemax="${g.players.length}"><div class="progress-fill" style="width:${done/g.players.length*100}%"></div></div>
    <div class="players">${g.players.map(x=>`<div class="player-row">${avatar(x)}<div class="player-info"><div class="player-name">${e(x.name)}${x.id===p.id?' <span class="subtle">(du)</span>':''}</div><div class="player-role">${[x.id===g.hostId?'Spielleitung':'',x.id===g.startPlayerId?'Startspieler':''].filter(Boolean).join(' · ')||'Mitspieler'}</div></div><span class="chip ${g.results[x.id].status==='submitted'?'green':''}">${g.results[x.id].status==='submitted'?'✓ Fertig':'Eingabe offen'}</span></div>`).join('')}</div>
    <div class="panel-footer"><p class="subtle">${closed?'Gespeichert auf diesem Gerät.':g.results[p.id].status==='submitted'?'Deine Eingabe ist abgeschlossen. Bis zur Bestätigung kannst du sie bearbeiten.':'Deine Werte fehlen noch. Du kannst sie in Ruhe ergänzen.'}</p><button class="button primary" data-view="${closed?'review':'entry'}">${closed?'Ergebnis ansehen':g.results[p.id].status==='submitted'?'Meine Werte ansehen':'Meine Werte eintragen'} <span aria-hidden="true">→</span></button></div></section>
    <section class="panel"><div class="panel-header"><h2>So geht’s weiter</h2></div><p class="subtle">${closed?'Du kannst jetzt eine neue Partie anlegen oder das Ergebnis ansehen.':p.id===g.hostId?'Sobald alle fertig sind, prüfst du die Bonuskarten und Siegpunkte. Danach kannst du die Partie bestätigen.':'Sobald alle fertig sind, prüft die Spielleitung die Bonuskarten und bestätigt die Partie.'}</p>${p.id===g.hostId&&!closed?'<button class="muted-link" data-view="review">Zur Abschlusskontrolle →</button>':''}${g.rules||g.tags||g.notes?`<details><summary>Angaben zur Partie</summary><p class="subtle">${e([g.rules,g.tags,g.notes].filter(Boolean).join(' · '))}</p></details>`:''}</section>
...
function entry(g,p){
  ...
  return `<div class="section-intro"><p class="eyebrow">${e(p.name)} · Eigene Eingabe</p><h2>Wie lief deine Partie?</h2><p class="subtle">Trage deinen Stand am Spielende ein.</p></div>${locked?'<div class="notice success">Diese Partie ist bestätigt. Deine Werte sind jetzt schreibgeschützt.</div>':''}
    <form id="entry-form"><section class="panel"><div class="panel-header"><h2>Dein Spielbrett</h2><span class="chip" id="entry-badge">${r.status==='submitted'?'✓ Eingabe fertig':'Entwurf'}</span></div>${FIELDS.filter(f=>f[0]!=='finale_siegpunkte').map(f=>stepper(f,r.values,locked)).join('')}</section>
    <section class="panel"><div class="panel-header"><h2>Dein Endstand</h2></div>${stepper(FIELDS.find(f=>f[0]==='finale_siegpunkte'),r.values,locked)}<div class="total-card"><div><strong>Aus deinen Werten berechnet</strong><p>Inklusive vorläufiger Bonuskarten. Die Spielleitung prüft diese am Ende.</p></div><span class="total-number" id="calculated">${points(g,p.id)}</span></div><div id="entry-errors" role="alert"></div><div class="save-bar"><div><p class="subtle" id="save-status">${r.updatedAt?'Auf diesem Gerät gespeichert.':'Beispielwerte · noch nicht abgeschlossen.'}</p><p class="subtle">${connection()}</p></div>${locked?'<button type="button" class="button" data-view="review">Zum Ergebnis →</button>':`<button class="button primary" type="submit">${r.status==='submitted'?'Eingabe erneut abschließen':'Eingabe abschließen'} <span aria-hidden="true">✓</span></button>`}</div></section></form>`;
}
...
function reviewView(g,p,host){
  ...
  return `<div class="section-intro"><p class="eyebrow">${closed?'Abgeschlossene Partie':'Gemeinsame Abschlusskontrolle'}</p><h2>${closed?'Das Ergebnis steht.':'Ein letzter Blick.'}</h2><p class="subtle">${closed?'Gespeichert auf diesem Gerät.':host?'Prüfe die Werte, bevor du die Partie bestätigst.':'Die Spielleitung prüft und bestätigt eure Partie.'}</p></div>
    ...
    <section class="panel"><div class="panel-header"><h2>Die Bonuskarten</h2></div><div class="award-grid">${awardBox(g,'road',host)}${awardBox(g,'army',host)}</div><p class="subtle" style="margin-top:14px;font-size:12px">Berechnung wie in deiner bisherigen App. Bei Gleichstand entscheidet die Spielleitung.</p></section>
...
function exportData(){const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`catan-probelauf-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Lokale Daten als JSON exportiert.');}
root.addEventListener('click',event=>{
  ...
  try {
    ...
    if(b.dataset.action==='confirm'){confirmGame(game(),state.actorId,Boolean(document.querySelector('#accept-mismatch')?.checked));const ok=persist();render();toast(ok?'Partie bestätigt und auf diesem Gerät gespeichert.':'Bestätigt, aber nicht gespeichert. Bitte Daten exportieren.');}
...
root.addEventListener('change',event=>{
  ...
  if(input.dataset.award){try{chooseAward(game(),state.actorId,input.dataset.award,input.value||null);persist();render();}catch(err){toast(err.message);}}
});
...
root.addEventListener('submit',event=>{
  ...
  try{updateResult(game(),state.actorId,readEntry(),{submit:true});const ok=persist();navigate('overview');toast(ok?'Deine Eingabe ist abgeschlossen.':'Eingabe abgeschlossen, aber nicht gespeichert. Bitte exportieren.');}
...
function init(){
  ...
  if(state.schemaVersion!==1||!Array.isArray(state.games)||!state.games.length)throw Error('Der lokale Spielstand ist nicht mit dieser Version kompatibel.');
  render();
