...
function loginView(){shell(`<section class="panel auth-panel"><p class="eyebrow">Eure Runde verbindet sich</p><h1>Zusammen spielen.</h1><p class="lead">Schnell an den Spieltisch – ganz ohne Passwort.</p><button class="button primary" data-cloud="guest">Als Gast mitspielen</button><p class="subtle">Du wählst deinen Namen beim Beitritt zur Partie. Dein Gastzugang bleibt in diesem Browser gespeichert. Nach Abmeldung oder Löschen der Browserdaten kannst du ihn nicht wiederherstellen.</p><hr><h2>Dein festes Spielerkonto</h2><p>Mit einem Link per E-Mail meldest du dich auch auf einem anderen Handy an.</p>${cloud.EMAIL_READY?`<form id="shared-auth"><label class="field">E-Mail<input name="email" type="email" required autocomplete="email"></label><button type="submit" class="button">Anmeldelink senden</button></form>`:'<p class="notice">Der E-Mail-Versand wird noch eingerichtet. Nutze solange den Gastzugang.</p>'}</section>`);}

function lobbyView(){shell(`<div class="page-heading"><div><p class="eyebrow">Deine Spielrunde</p><h1>Gemeinsame Partien</h1><p class="subtle">${esc(user().is_anonymous?'Gast · auf diesem Browser gespeichert':user().email||'Angemeldet')}</p></div><button class="button primary" data-cloud="new">＋ Neue Partie</button></div><section class="panel"><div class="panel-header"><h2>Deine Partien</h2><button class="button small" data-cloud="refresh">Aktualisieren</button></div><div class="history-list">${games.length?games.map(g=>`<button class="history-item" data-open="${g.id}"><span><strong>${esc(g.place||'Catan-Runde')}</strong><small>${esc(g.date)} · ${g.players.length} Spieler</small></span><span class="chip ${g.status==='confirmed'?'green':''}">${g.status==='confirmed'?'Bestätigt':`${g.players.filter(p=>g.results[p.id].status==='submitted').length}/${g.players.length} fertig`}</span></button>`).join(''):'<p class="subtle">Noch keine gemeinsame Partie. Lege eine an oder öffne den Einladungslink deiner Spielleitung.</p>'}</div></section><section class="panel"><h2>Einladung erhalten?</h2><form id="shared-invite"><label class="field">Einladungslink oder Einladungscode<input name="token" required placeholder="Einladungslink einfügen" autocomplete="off"></label><button type="submit" class="button">Partie finden</button></form></section>`);}
...
root.addEventListener('click',event=>{
 const button=event.target.closest('[data-cloud],[data-open],[data-join],[data-shared-step]');if(!button||button.disabled)return;event.preventDefault();
...
async function start(){
 ...
 try{await cloud.consumeCallback();if(user()){try{games=JSON.parse(localStorage.getItem(cacheKey())||'[]');}catch{}if(navigator.onLine){await sync();const t=localStorage.getItem('catan-invitation');if(t)await preview(t);}}}catch(err){message=err.message;}
 render();
